...
`;
    ...
    /**
 * Generates a PowerShell one-liner command string to create a CSV file and run the PWD L script.
 *
 * @param {Array<Object>} listOfStates - An array of data objects to convert to CSV.
 * Each object is expected to have properties like name, batch_name, id,
 * and potentially topic_name, lecture_url, etc.
 * @param {object} [options={}] - Optional parameters.
 * @param {string} [options.pythonCmd="python"] - The command to use for the Python interpreter (e.g., "python3", "py.exe"). Defaults to "python".
 * @param {string} [options.pwdlScriptPath="$env:USERPROFILE\\pwdlv3\\pwdl.py"] - The path to the PWD L Python script. Defaults to "$env:USERPROFILE\\pwdlv3\\pwdl.py".
 * @returns {string} The PowerShell one-liner command string, or an empty string if input is invalid or encoding fails.
 */
static toPwdlCommandPowerShell(listOfStates, options = {}) {
    if (!Array.isArray(listOfStates) || listOfStates.length === 0) {
        console.error("toPwdlCommandPowerShell: Invalid input - listOfStates must be a non-empty array.");
        return "";
    }
    ...
    const csvData = Utils.convertToCSV(listOfStates);
    if (!csvData) {
        console.error("toPwdlCommandPowerShell: Failed to convert data to CSV.");
        return "";
    }
    ...
    // Escape CSV data for use in PowerShell here-string.
    // Escape backticks and dollar signs to prevent premature interpretation
    const escapedCsvData = csvData.replace(/`/g, '``').replace(/\$/g, '`$');

    // --- Construct the multi-line PowerShell script content template ---
    // Values from JS options will be substituted into this template.
    const multiLineScriptTemplate = `# This script is generated by the Utils.toPwdlCommandPowerShell JavaScript function.
# It will be created and executed in the current directory.
...
$CSV_FILE_NAME = "${csvFileName}"
# Full path to the CSV file within the subdirectory
$CSV_FILE_PATH = Join-Path $DIR_NAME $CSV_FILE_NAME

try {
    # Create the data subdirectory if it doesn't exist
    Write-Host "Creating data directory: $DIR_NAME..." -ForegroundColor Green
    if (!(Test-Path $DIR_NAME)) {
        New-Item -ItemType Directory -Path $DIR_NAME -Force | Out-Null
    }

    # Create CSV file using a here-string inside the data subdirectory
    Write-Host "Creating CSV file: $CSV_FILE_PATH..." -ForegroundColor Green
    $csvContent = @"
${escapedCsvData}
"@
    Set-Content -Path $CSV_FILE_PATH -Value $csvContent -Encoding UTF8

    # Check if CSV file was created successfully
    if (Test-Path $CSV_FILE_PATH) {
        Write-Host "CSV file created successfully: $CSV_FILE_PATH" -ForegroundColor Green
        Write-Host "Running PWD L command..." -ForegroundColor Yellow
        
        # Check if the Python script exists at the expected path before executing
        if (!(Test-Path $PYTHON_SCRIPT)) {
            Write-Error "PWD L script not found at expected path: $PYTHON_SCRIPT"
            Write-Host "Please verify the path or manually edit the generated script file." -ForegroundColor Red
            exit 1
        }
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "PWD L completed successfully." -ForegroundColor Green
        } else {
            Write-Error "PWD L failed (exit code $LASTEXITCODE). Please check the output above."
            exit $LASTEXITCODE
        }
    } else {
        Write-Error "Failed to create CSV file at $CSV_FILE_PATH."
        exit 1
    }
} catch {
...
`;
    ...
    // --- Construct the final PowerShell one-liner command ---
    // This command will:
    // 1. Create a unique temporary script file name in the current directory
    // 2. Decode the base64 string and save it to the temporary file
    // 3. Execute the temporary script using PowerShell
    // 4. Remove the temporary script file after execution (using finally block)
    const oneLinerCommand = `$TempFile = [System.IO.Path]::GetTempFileName() + '.ps1'; try { [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String('${base64Script}')) | Set-Content -Path $TempFile -Encoding UTF8; & powershell -ExecutionPolicy Bypass -File $TempFile } finally { if (Test-Path $TempFile) { Remove-Item $TempFile -Force } }`;

    return oneLinerCommand;
}

/**
 * Generates a Command Prompt (batch) one-liner command string to create a CSV file and run the PWD L script.
 *
 * @param {Array<Object>} listOfStates - An array of data objects to convert to CSV.
 * Each object is expected to have properties like name, batch_name, id,
 * and potentially topic_name, lecture_url, etc.
 * @param {object} [options={}] - Optional parameters.
 * @param {string} [options.pythonCmd="python"] - The command to use for the Python interpreter (e.g., "python3", "py.exe"). Defaults to "python".
 * @param {string} [options.pwdlScriptPath="%USERPROFILE%\\pwdlv3\\pwdl.py"] - The path to the PWD L Python script. Defaults to "%USERPROFILE%\\pwdlv3\\pwdl.py".
 * @returns {string} The batch command string, or an empty string if input is invalid or encoding fails.
 */
static toPwdlCommandBatch(listOfStates, options = {}) {
    if (!Array.isArray(listOfStates) || listOfStates.length === 0) {
        console.error("toPwdlCommandBatch: Invalid input - listOfStates must be a non-empty array.");
        return "";
    }
    ...
    const csvData = Utils.convertToCSV(listOfStates);
    if (!csvData) {
        console.error("toPwdlCommandBatch: Failed to convert data to CSV.");
        return "";
    }
    ...
    // Escape CSV data for batch file - escape % and ^ characters
    const escapedCsvData = csvData.replace(/%/g, '%%').replace(/\^/g, '^^');

    // --- Construct the multi-line batch script content template ---
    const multiLineScriptTemplate = `@echo off
REM This script is generated by the Utils.toPwdlCommandBatch JavaScript function.
REM It will be created and executed in the current directory.

setlocal EnableDelayedExpansion

REM Use the Python interpreter command provided by the generating function
set "PYTHON_CMD={{PYTHON_CMD}}"
REM Use the PWD L script path provided by the generating function
set "PYTHON_SCRIPT={{PYTHON_SCRIPT}}"

REM The directory for the CSV data, relative to where this script is run
set "DIR_NAME=${directoryName}"
set "CSV_FILE_NAME=${csvFileName}"
REM Full path to the CSV file within the subdirectory
set "CSV_FILE_PATH=%DIR_NAME%\\%CSV_FILE_NAME%"

REM Create the data subdirectory if it doesn't exist
echo Creating data directory: %DIR_NAME%...
if not exist "%DIR_NAME%" mkdir "%DIR_NAME%"

REM Create CSV file inside the data subdirectory
echo Creating CSV file: %CSV_FILE_PATH%...
(
${escapedCsvData}
) > "%CSV_FILE_PATH%"

REM Check if CSV file was created successfully
if exist "%CSV_FILE_PATH%" (
    echo CSV file created successfully: %CSV_FILE_PATH%
    echo Running PWD L command...
    
    REM Check if the Python script exists at the expected path before executing
    ...
    REM Execute the pwdl script using the specified interpreter and path
    "%PYTHON_CMD%" "%PYTHON_SCRIPT%" --dir "%DIR_NAME%" --csv-file "%CSV_FILE_PATH%" --verbose
    
    REM Check the exit status of the pwdl command
    if !errorlevel! equ 0 (
        echo PWD L completed successfully.
    ) else (
        echo PWD L failed ^(exit code !errorlevel!^). Please check the output above.
        exit /b !errorlevel!
    )
) else (
    echo Error: Failed to create CSV file at %CSV_FILE_PATH%.
...
`;
    ...
    // --- Construct the final batch one-liner command ---
    // This uses PowerShell within CMD to handle Base64 decoding since batch doesn't have native Base64 support
    const oneLinerCommand = `for /f "tokens=*" %i in ('powershell -Command "[System.Guid]::NewGuid().ToString() + '.bat'"') do set "TempFile=%i" && powershell -Command "[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String('${base64Script}')) | Set-Content -Path '%TempFile%' -Encoding UTF8" && call "%TempFile%" && del "%TempFile%"`;
